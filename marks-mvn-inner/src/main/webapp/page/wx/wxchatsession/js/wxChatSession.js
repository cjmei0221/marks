var appInfo = {
	listUrl: top.window.urlBase + '/wxChatSession/list.do',//获取回话管理列表接口  WxChatSession
	...
	$("#delete").on("click",function(){
		if(isSelectedOne(appInfo.selectedId)){
			$.messager.confirm('确认', '确认要删除该记录吗?', function(r) {
				if (r) {
					var parms = "session_id=" + appInfo.selectedId;
...
function loadList() {
	$('#tbList').datagrid({
		...
		singleSelect : true,
		columns : [ [                 {title:'回话ID',field:'session_id',width:100,align:"center",hidden:true },
                ...
                {title:'创建时间戳',field:'createLong',width:100,align:"center"},
                {title:'更新时间',field:'updatetime',width:100,align:"center"},
                {title:'内容',field:'c_content',width:100,align:"center"},
                {title:'结束时间',field:'endtime',width:100,align:"center"},
                {title:'粉丝ID',field:'fanId',width:100,align:"center"} ] ],
		loader : function(params, success, loadError) {
			var that = $(this);
			loader(that, params, success, loadError);
		},
		onClickRow : function(rowIndex,rowData) {
			appInfo.selectedId = rowData.session_id;
