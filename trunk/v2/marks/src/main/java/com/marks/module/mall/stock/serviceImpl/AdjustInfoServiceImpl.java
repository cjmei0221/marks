...
import com.marks.module.mall.stock.dao.AdjustGoodDao;
import com.marks.module.mall.stock.dao.AdjustInfoDao;
		...
		if (Enums.CheckStatus.checkOk.toString().equals(checkStatus)) {
			...
			if (null != list && list.size() > 0) {
				...
				for (AdjustGood good : list) {
					nums = good.getNums();
					stockId = "I-" + info.getCompanyId() + "-" + info.getOrgId() + "-" + good.getGoodNo();
					stock = stockInfoDao.findById(stockId);
					if (stock != null) {
