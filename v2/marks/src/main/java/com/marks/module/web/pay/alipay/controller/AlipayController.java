package com.marks.module.web.pay.alipay.controller;
...
import com.marks.module.web.pay.alipay.alipay.Alipay_Wap;
import com.marks.module.web.pay.alipay.alipay.util.AlipayNotify;
...
public class AlipayController {
	...
	public void pay(HttpServletRequest request,HttpServletResponse response) {
		...
		try {
			...
			html = Alipay_Wap.wap_consume(paramMap,0);
	...
	public void pay2(HttpServletRequest request,HttpServletResponse response) {
		...
		try {
			...
			paramMap.put("subject", "奶粉");
			paramMap.put("total_fee", NumberUtil.formatNumber(txnAmtd,"0.00"));
			html = Alipay_Wap.wap_consume(paramMap,1);
		...
		try {
			...
			if(AlipayNotify.verify(paramMap)){
				...
				if("T".equals(is_success) && ("TRADE_FINISHED".equals(trade_status) ||"TRADE_SUCCESS".equals(trade_status))){
					String paramsUrl = AlipayPropUtil.getProperty("pay_success_page_url");
					String params = "orderId="+orderId+"&orderprice="+total_fee;
					String orderPageUrl =paramsUrl+"?"+params;
					response.sendRedirect(orderPageUrl);
				}else{
					logger.info("交易失败");
					String paramsUrl =AlipayPropUtil.getProperty("pay_fail_page_url");
					String params = "orderId="+orderId+"&orderprice="+total_fee;
					String orderPageUrl =paramsUrl+"?"+params;
					response.sendRedirect(orderPageUrl);
				}
