...
public class FeeTypeController extends SupportContorller{
    ...
    public void findFeeTypeById(HttpServletRequest request,
    ...
    public void saveFeeType(HttpServletRequest request,
    HttpServletResponse response){
		Result result = new Result();
		try {
    ...
    public void updateFeeType(HttpServletRequest request,
    HttpServletResponse response){
		Result result = new Result();
		try {
			SysUser admin = LoginUtil.getInstance().getCurrentUser(request);
    ...
    public void deleteFeeTypeById(HttpServletRequest request,
    HttpServletResponse response){
		Result result = new Result();
		try {
		   	FeeType info = getModel(FeeType.class);
    ...
    /*
    @RequestMapping("/inner/feeType/findAllFeeType")
    public void findAllFeeType(HttpServletRequest request,
    HttpServletResponse response){
		Result result = new Result();
		try {
	...
	*/
	 ...
	 */
	/*
	@RequestMapping("/inner/feeType/deleteIds")
	public void deleteFeeType(HttpServletRequest request,
			HttpServletResponse response){
		Result result = new Result();
		try {
			...
			if(idList.size()>0){
				feeTypeService.deleteBatch(idList);
				result.setMessage("删除成功!");
				result.setCode(Code.CODE_SUCCESS);
			}else{
				result.setMessage("删除失败，请联系管理员!");
