package com.marks.module.web.pay.wxpay.controller;
...
public class WXPayController extends SupportContorller{
	...
	@RequestMapping("/WXPay/notifyStoreDeposit")
	public void wxNotifyStoreDeposit(HttpServletRequest request, HttpServletResponse response) {
		WXPayUtil wxPayUtil = new WXPayUtil();
		...
		try {
			...
			if (resultParams.containsKey("return_code") && "FAIL".equals(resultParams.get("return_code"))) {
				logger.info("-------------wxpay notify result_code:" + resultParams.get("result_code") + ";return_msg:"
				...
				logger.info("-------------wxpay notify result_code:" + notice.getResult_code() + ";appid:"
			...
			logger.info("微信回掉函数出现错误(门店保证金):", e);
			return;
		}
		logger.info("--------------------wx pay notify data(门店保证金):" + xml);
